...
- **Results grid:** `repeat(auto-fill, minmax(272px,1fr))`, gap 26×24px. Each card
  (square corners, 0.5px border, hover-lift). The design is **fully image-free** — see
  *Imagery* — so there is NO photo well and no placeholder anywhere:
  - **Header band** (compact, ~16×20px padding — NOT a tall image plate): dark
    `linear-gradient(158deg,#201d19→#131211)` + a brass radial glow in the top-right,
    holding an italic serif `N° 01` catalogue number (left) and a translucent "HOMAGE"
    pill (right).
...
## Assets / Data
- **No image assets, and none will ever exist — the design is intentionally image-free.**
  The client has confirmed they have no photography and will not have any. The hero and
  cards are pure CSS (layered gradients + serif typography) with **no photo wells, image
  slots, or placeholders** anywhere. Do NOT reintroduce an image area, a faux-photo
  plate, or an "add a photo" affordance when porting — the typographic treatment is the
  final, complete design. (An earlier iteration had a tall 4:5 tonal card plate with an
  "ADD YOUR PHOTOGRAPH" hint; it was removed and replaced with the compact typographic
  header band described above. `image-slot.js` is no longer needed and is not part of
  this deliverable.)
